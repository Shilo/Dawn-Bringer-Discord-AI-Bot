...
class RAGChain:
    ...
    def __init__(
        ...
        self.system_prompt = (
            system_prompt or "You are Dawn Bringer, a helpful Discord AI assistant."
        )
    ...
    async def _prepare_query(
        ...
        skip_rag_retrieval = additional_metadata and additional_metadata.get(
            "skip_rag_retrieval", False
        )
        ...
        threshold = (
            ...
            else self.retriever.vector_store.config.SCORE_THRESHOLD
        )
        ...
        if skip_rag_retrieval:
            ...
            # Update status to show we're starting vector search
            if status_updater:
                await status_updater(StatusMessage.SEARCHING_VECTOR_DATABASE)

            if include_scores:
                # Single search that returns both docs and scores
                try:
                    scores = self.retriever.retrieve_with_scores(
                        user_query,
                    ...
                    retrieved_docs = self.retriever.retrieve(
                        user_query, apply_threshold=True, top_k_override=top_k_override
                    )
                    scores = None
            else:
            ...
            # Update status to show retrieval is complete
            if status_updater:
                await status_updater(StatusMessage.PROCESSING_RETRIEVED_DOCS)
        ...
        if additional_context:
            ...
            dynamic_doc = LangChainDocument(
                page_content=additional_context, metadata=metadata
            )
    ...
    async def query(
        ...
        """
        retrieved_docs, message_content, sources, _ = await self._prepare_query(
            user_query,
        ...
        code_block_pattern = (
            r'```(?:json)?\s*(\{[^{}]*"used_sources"[^{}]*\[[^\]]*\][^{}]*\})\s*```'
        )
        code_block_match = re.search(
            code_block_pattern, response_text, re.DOTALL | re.IGNORECASE
        )
        if code_block_match:
            try:
    ...
    async def query_with_usage(
        ...
        current_status_updater = (
            status_updater if status_updater is not None else self.status_updater
        )

        retrieved_docs, message_content, sources, scores = await self._prepare_query(
            user_query,
            ...
            additional_context=additional_context,
            additional_metadata=additional_metadata,
            status_updater=current_status_updater,
        )
        ...
        if current_status_updater:
            await current_status_updater(StatusMessage.GENERATING_RESPONSE)
        ...
        if current_status_updater:
            await current_status_updater(StatusMessage.PROCESSING_AI_RESPONSE)
        ...
        for idx, doc in enumerate(
            retrieved_docs, 1
        ):  # Start from 1 to match Source 1, Source 2, etc.
            source = doc.metadata.get("source", "Unknown")
            score = scored_docs_map.get(
                source
            )  # Get score if available, None otherwise
